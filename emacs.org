...
* IDE
** LSP
Currently ts-ls and deno lsp servers can't coexist. So you need to uninstall typescript globally when using
deno.

npm un -g typescript-language-server; npm un -g typescript

If uninstalling typescripRemember to clear the cache inside .emacs.d/.cache/lsp

To re-use ts-ls server, simply reinstall globally
#+begin_src emacs-lisp
...
** Helm Ag
Note to filter by file:

-G<file> <search-term> -GSignForm useNavigate
<search-term> -G<file> useNavigate -GSignform
